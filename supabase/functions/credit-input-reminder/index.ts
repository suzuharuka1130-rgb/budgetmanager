...
// 文面は世帯ごとに app_settings.reminder_credit_text へ保存。未設定（空）の世帯は送信しない。
import { sendLineMessage, listHouseholdIds, householdLineRecipients } from '../_shared/line.ts'
import { getServiceClient, isLastDayOfMonthJST, getHouseholdSetting } from '../_shared/report.ts'
...
Deno.serve(async () => {
  ...
  try {
    ...
    for (const hid of hids) {
      const message = (await getHouseholdSetting(sb, hid, 'reminder_credit_text'))?.trim()
